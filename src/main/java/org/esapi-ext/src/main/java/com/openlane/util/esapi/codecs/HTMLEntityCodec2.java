package com.openlane.util.esapi.codecs;
...
public class HTMLEntityCodec2 extends Codec
{
    ...
    private static final Logger log = LoggerFactory.getLogger(HTMLEntityCodec2.class);
    /**
     *
     */
    public HTMLEntityCodec2() {
    }
    ...
    public String encodeCharacter( char[] immune, Character c ) {
        ...
        String hex = instance.getHexForNonAlphanumeric(c);
        if ( hex == null ) {
            return ""+c;
        }
    ...
    public String decode(String input) {
        ...
        while (pbs.hasNext()) {
            Character c = decodeCharacter2(pbs);
     ...
     *   &name;
     */
    public Character decodeCharacter2( PushbackString input ) {
        input.mark();
        ...
        if (second == '#' ) {
            ...
            Character c = getNamedEntity2( input );
            if ( c != null ) return c;
        }
        input.reset();
        return null;
    ...
    private Character getNamedEntity2( PushbackString input ) {
        ...
        // now check if match has ; as trail
        String m = entry.getKey().toString();
        log.info("possible encode found: &" + m + " (in  " + possible.toString() +")");
        if (!possible.toString().startsWith(m+";")) return null;

        log.info("encode : &" + m + "; match (in  " + possible.toString() +")");
        // fixup input
        input.reset();
        input.next();	// read &
     ...
     */
    private Character getNumericEntity( org.owasp.esapi.codecs.PushbackString input ) {
     ...
     */
    private Character parseNumber( org.owasp.esapi.codecs.PushbackString input ) {
        StringBuilder sb = new StringBuilder();
        while( input.hasNext() ) {
     ...
     * @throws NumberFormatException
     */
    private Character parseHex( org.owasp.esapi.codecs.PushbackString input ) {
        StringBuilder sb = new StringBuilder();
        while( input.hasNext() ) {
            Character c = input.peek();
